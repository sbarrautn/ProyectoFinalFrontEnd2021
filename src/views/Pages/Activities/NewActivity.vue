<template>
  <div>
    <div class="header pb-8 pt-5 pt-lg-8 d-flex align-items-center profile-header"
         style="min-height: 600px; background-image: url(img/theme/students-in-classroom-792x418.jpg); background-size: cover; background-position: center top;">
      <b-container fluid>
        <!-- Mask -->
        <span class="mask bg-gradient-success opacity-8"></span>
        <!-- Header container -->
        <b-container fluid class="d-flex align-items-center">
          <b-row>
            <b-col lg="7" md="10">
              <h1 class="display-2 text-white">Gestión de cursado</h1>
              <p class="text-white mt-0 mb-5">Aquí puedes llevar el control de los cursos</p>
            </b-col>
          </b-row>
        </b-container>
      </b-container>
    </div>

    <b-container fluid class="mt--6">
      <b-row>
        <div class="custom-card">
          <card>
            <b-row align-v="center" slot="header">
              <b-col cols="8">
                <h3 class="mb-0">Nueva actividad</h3>
              </b-col>
            </b-row>

            <b-form @submit.prevent="updateProfile">
              <h6 class="heading-small text-muted mb-4">Datos de la actividad</h6>

              <div class="pl-lg-4">
                <b-row>
                  <b-col lg="8">
                    <base-input
                      type="text"
                      label="Título"
                      placeholder="Título de la actividad"
                      v-model="activity.title"
                    >
                    </base-input>
                  </b-col>
                </b-row>
                <b-row>
                  <b-col lg="12">
                    <base-input
                      type="text"
                      label="Descripción"
                      placeholder="Descripción de la actividad"
                      v-model="activity.description"
                    >
                    </base-input>
                  </b-col>
                </b-row>
              </div>

              <h6 class="heading-small text-muted mb-4">Apartados de la actividad</h6>

              <div>
                <b-row>
                  <b-col cols="1" class="text-right">
                    <base-button type="primary" native-type="submit" class="my-4">Agregar apartado</base-button>
                  </b-col>
                </b-row>

                <b-row>
                  <b-col lg="12">
                    <base-input
                      type="text"
                      label="Pregunta"
                      placeholder="Pregunta"
                    >
                    </base-input>
                  </b-col>
                  <b-col lg="12">
                    <b-container>
                      <b-row>
                        <base-input label="Single select">
                          <el-select v-model="activity.selectedValue" filterable
                                     placeholder="Simple select">
                            <el-option v-for="option in activity.options"
                                       :key="option.label"
                                       :label="option.label"
                                       :value="option.value">
                            </el-option>
                          </el-select>
                        </base-input>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <h6 class="heading-small text-muted mb-4">Correcta</h6>
                        </b-col>
                        <b-col lg="10">
                          <h6 class="heading-small text-muted mb-4">Respuesta</h6>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <base-radio></base-radio>
                        </b-col>
                        <b-col lg="10">
                          <base-input
                            type="text"
                            placeholder="Respuesta"
                          >
                          </base-input>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <base-radio></base-radio>
                        </b-col>
                        <b-col lg="10">
                          <base-input
                            type="text"
                            placeholder="Respuesta"
                          >
                          </base-input>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <base-radio></base-radio>
                        </b-col>
                        <b-col lg="10">
                          <base-input
                            type="text"
                            placeholder="Respuesta"
                          >
                          </base-input>
                        </b-col>
                      </b-row>
                    </b-container>
                  </b-col>
                </b-row>

                <b-row>
                  <b-col lg="12">
                    <base-input
                      type="text"
                      label="Pregunta"
                      placeholder="Pregunta"
                    >
                    </base-input>
                  </b-col>
                  <b-col lg="12">
                    <b-container>
                      <b-row>
                        <base-input label="Single select">
                          <el-select v-model="activity.selectedValue" filterable
                                     placeholder="Simple select">
                            <el-option v-for="option in activity.options"
                                       :key="option.label"
                                       :label="option.label"
                                       :value="option.value">
                            </el-option>
                          </el-select>
                        </base-input>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <h6 class="heading-small text-muted mb-4">Correcta</h6>
                        </b-col>
                        <b-col lg="10">
                          <h6 class="heading-small text-muted mb-4">Respuesta</h6>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <base-radio></base-radio>
                        </b-col>
                        <b-col lg="10">
                          <h6 class="heading-small text-muted mb-4">Verdadero</h6>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <base-radio></base-radio>
                        </b-col>
                        <b-col lg="10">
                          <h6 class="heading-small text-muted mb-4">Falso</h6>
                        </b-col>
                      </b-row>
                    </b-container>
                  </b-col>
                </b-row>

                <b-row>
                  <b-col lg="12">
                    <base-input
                      type="text"
                      label="Pregunta"
                      placeholder="Pregunta"
                    >
                    </base-input>
                  </b-col>
                  <b-col lg="12">
                    <b-container>
                      <b-row>
                        <base-input label="Single select">
                          <el-select v-model="activity.selectedValue" filterable
                                     placeholder="Simple select">
                            <el-option v-for="option in activity.options"
                                       :key="option.label"
                                       :label="option.label"
                                       :value="option.value">
                            </el-option>
                          </el-select>
                        </base-input>
                      </b-row>
                      <b-row>
                        <b-col lg="2">
                          <h6 class="heading-small text-muted mb-4">Correcta</h6>
                        </b-col>
                        <b-col lg="10">
                          <h6 class="heading-small text-muted mb-4">Respuesta</h6>
                        </b-col>
                      </b-row>
                      <b-row>
                        <b-col lg="12">
                          <base-input
                            type="text"
                            placeholder="Respuesta"
                          >
                          </base-input>
                        </b-col>
                      </b-row>
                    </b-container>
                  </b-col>
                </b-row>
              </div>

              <div>
                <b-row>
                  <b-col cols="1" class="text-right">
                    <base-button type="primary" native-type="submit" class="my-4">Crear actividad</base-button>
                  </b-col>
                </b-row>
              </div>

            </b-form>
          </card>
        </div>
      </b-row>
    </b-container>
  </div>
</template>

<script>
import router from "../../../routes/router";
import {Select, Option} from 'element-ui'
import activityExample from './../../Tables/activityExample'

export default {
  components: {
    [Select.name]: Select,
    [Option.name]: Option
  },
  data() {
    return {
      activity: {
        options: [
          {
            label: 'multiple opción',
            value: 'val1'
          },
          {
            label: 'verdadero o falso',
            value: 'val2'
          },
          {
            label: 'redacción',
            value: 'val3'
          }
        ],
        selectedValue: '',
        title: '',
        description: '',
        type: 1,
        body: JSON.stringify(activityExample),
        task: 1,
        successMsg: 'Actividad creada correctamente!',
        conflictMsg: 'Ya existe una cuenta con esa dirección de correo asociada!'
      }
    };
  },
  methods: {
    updateProfile() {
      let session = localStorage.getItem('session');
      if (!session) {
        session = sessionStorage.getItem('session');
      }
      axios.post('http://api.proyecto.test/api/activities', {
        title: this.activity.title,
        type: this.activity.type,
        description: this.activity.description,
        body: this.activity.body,
        task: this.activity.task,
      }, {
        headers: {
          'Authorization': `${session}`
        }
      })
        .then((response) => {
          if (response.data.http_code === 201) {
            this.$notify({
              type: 'success',
              verticalAlign: 'bottom',
              horizontalAlign: 'center',
              message: this.model.successMsg
            });
          }
        })
        .catch((error) => {
          if (error.response.data.http_code === 409) {
            this.$notify({
              type: 'warning',
              verticalAlign: 'bottom',
              horizontalAlign: 'center',
              message: this.model.conflictMsg
            });
          } else {
            this.$notify({type: 'danger', verticalAlign: 'bottom', horizontalAlign: 'center', message: error.message});
          }
        });
    }
  },
  beforeCreate() {
    let session = localStorage.getItem('session');
    if (!session) {
      session = sessionStorage.getItem('session');
    }
    if (!session) {
      router.push({name: 'login'});
    }
  }
};
</script>

<style>
.custom-card {
  min-width: 100% !important;
  margin-bottom: 70px;
}
</style>
